...
function FindJob() {
    ...
    useEffect(() => {
        var url = "";
        if (!filterCategory) {
            url = `adserver/allads?pageNumber=${pageNumber}&pageSize=10&excludeAuthorId=${userId}`
        }
        else {
            url = `adserver/adsbycategory?pageNumber=${pageNumber}&pageSize=10&excludeAuthorId=${userId}&categoryId=${filterCategory.categoryId}`
        }
